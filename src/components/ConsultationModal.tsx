        ...
        >
          ...
          >
            ...
            <form onSubmit={submit} className="space-y-3 p-5">
              <p className="text-[10px] leading-relaxed text-gray-300">{isAvailability ? "Masukkan tanggal yang Anda inginkan. Tim Mentari akan mengonfirmasi ketersediaan dan alternatif tanggal melalui WhatsApp." : "Isi detail singkat agar tim Mentari dapat menyiapkan estimasi dan proposal yang lebih relevan untuk Anda."}</p>
              <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                {!isAvailability && <label className="block"><span className="mb-1 block font-cinzel text-[8px] font-bold tracking-[0.12em] text-gold uppercase">Estimasi pax</span><input value={pax} onChange={(event) => setPax(event.target.value)} className="w-full rounded-lg border border-gold/30 bg-white/[0.06] px-3 py-2 text-xs text-white outline-none placeholder:text-gray-600 focus:border-gold" placeholder="Contoh: 500 pax" /></label>}
              </div>
              ...
              {!isAvailability && <label className="block"><span className="mb-1 block font-cinzel text-[8px] font-bold tracking-[0.12em] text-gold uppercase">Budget range</span><input value={budget} onChange={(event) => setBudget(event.target.value)} className="w-full rounded-lg border border-gold/30 bg-white/[0.06] px-3 py-2 text-xs text-white outline-none placeholder:text-gray-600 focus:border-gold" placeholder="Contoh: Rp100–150 juta" /></label>}
              <button type="submit" className="mt-1 inline-flex w-full items-center justify-center gap-2 rounded-full gold-gradient-bg px-5 py-3 text-xs font-semibold tracking-[0.1em] text-white"><MessageCircle className="h-4 w-4" /> {isAvailability ? "CEK KETERSEDIAAN DI WHATSAPP" : "MINTA PROPOSAL DI WHATSAPP"}</button>
            </form>
          </motion.div>
        </motion.div>
      )}
